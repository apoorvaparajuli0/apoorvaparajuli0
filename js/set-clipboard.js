const clipboardPaths = [
    "M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1z",
    "M9.5 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0z"
];

const checkedClipboardPaths = [
    "M10.854 7.146a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 1 1 .708-.708L7.5 9.793l2.646-2.647a.5.5 0 0 1 .708 0",
    "M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1z",
    "M9.5 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0z"
];

document.getElementById("email-link").addEventListener("click", function(ev) {
    navigator.clipboard.writeText("apoorvaparajuli@duck.com");

    let svg = document.querySelector(".profile-link .email-copy");
    const ns = "http://www.w3.org/2000/svg";

    svg.replaceChildren(
        ...checkedClipboardPaths.map((d, index) => {
            const p = document.createElementNS(ns, "path");
            p.setAttribute("d", d);
            if (index == 0) p.setAttribute("fill-rule", "evenodd");
            return p;
        })
    );

    setTimeout(() => {
        svg.replaceChildren(
            ...clipboardPaths.map((d, index) => {
                const p = document.createElementNS(ns, "path");
                p.setAttribute("d", d);
                return p;
            })
        );

    }, 2000)
    
})

document.getElementById("phone-link").addEventListener("click", async function(ev) {
    navigator.clipboard.writeText("510-374-9867");

    let svg = document.querySelector(".profile-link .phone-copy");
    const ns = "http://www.w3.org/2000/svg";

    svg.replaceChildren(
        ...checkedClipboardPaths.map((d, index) => {
            const p = document.createElementNS(ns, "path");
            p.setAttribute("d", d);
            if (index == 0) p.setAttribute("fill-rule", "evenodd");
            return p;
        })
    );

    setTimeout(() => {
        svg.replaceChildren(
            ...clipboardPaths.map((d, index) => {
                const p = document.createElementNS(ns, "path");
                p.setAttribute("d", d);
                return p;
            })
        );

    }, 2000)
})